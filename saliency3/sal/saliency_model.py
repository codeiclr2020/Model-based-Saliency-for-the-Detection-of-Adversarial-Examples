...
from saliency3.sal.utils.mask import *
...
class SaliencyModel(Module):
    ...
    def forward(self, _images, _selectors=None, pt_store=None, model_confidence=0.):
        ...
        if self.allow_selector:
            ...
            #for original use 256
            act = torch.sum(main_flow*em.view(-1, 256, 1, 1), 1, keepdim=True)
...
class SaliencyLoss:
    def __init__(self, black_box_fn, area_loss_coef=8.0, smoothness_loss_coef=0.0, preserver_loss_coef=0.3,
                 num_classes=1000, area_loss_power=0.3, preserver_confidence=1, destroyer_confidence=5, **apply_mask_kwargs):
        #removed smoothness_coef (default is 0.5)
        #area_loss default is 8
        #preserver_loss_co
    ...
    def get_loss(self, _images, _targets, _masks, _is_real_target=None, pt_store=None):
        ...
        if not isinstance(preserved_logits, torch.Tensor): #patch in case you don't use resnet black box
            preserved_logits = preserved_logits[-1]
            destroyed_logits = destroyed_logits[-1]
        ...
        total_loss = destroyer_loss + self.area_loss_coef*area_loss + self.preserver_loss_coef*preserver_loss + self.smoothness_loss_coef*smoothness_loss
