...
def get_train_dataset(size=32):
    return  CIFAR10('./saliency/data_cifar/', train=True, transform=Compose(
...
def get_val_dataset(size=32):
    return CIFAR10('./saliency/data_cifar/', train=False, transform=Compose(
...
CLASS_ID_TO_NAME = {i: CLASSES[i] for i in range(len(CLASSES))}
