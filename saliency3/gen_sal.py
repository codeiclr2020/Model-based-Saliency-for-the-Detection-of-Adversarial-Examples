...
if __name__ == '__main__':
    ...
    for set in range(0,12,1):
        # load in data
        index = set*1000
        with h5py.File(directory +'P'+ str(index), 'r') as f:
            X_train = f['Train']['X'].value
            Y_train = f['Train']['Pred'].value
        ...
        for j in range(4):
            ...
            for k in range(min(len(X_temp), len(Y_t))):
                # iterate over images
        ...
        with h5py.File(directory + 'A_Trial'+ SAVE_SAL_MODEL + str(index), 'w') as f:
