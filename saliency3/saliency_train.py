...
from sal.datasets import imagenet_dataset, cifar_dataset
...
saliency = SaliencyModel(resnet50encoder(pretrained=True), 5, 64, 3, 64, fix_encoder=True, use_simple_activation=False, allow_selector=True, num_classes=NUM_CLASSES)

saliency_p = nn.DataParallel(saliency).cuda()
saliency_loss_calc = SaliencyLoss(black_box_fn, smoothness_loss_coef=0.00, num_classes=NUM_CLASSES) # model based saliency requires very small smoothness loss and therefore can produce very sharp masks
...
saliency.minimalistic_save('yoursaliencymodel')  # later to restore just use saliency.minimalistic_restore method.